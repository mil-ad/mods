...
// This file renders LaTeX block math ($$…$$ and \[…\]) as images in mods'
// output — both the interactive TUI and non-interactive streaming. glamour has
// no math support and exposes no extension hook, so we work around it: block
// math is swapped for a sentinel before glamour runs and the sentinel is
// replaced with a kitty placeholder grid afterwards. The images come from the
// CodeCogs service, displayed via the kitty graphics protocol (see
// kittygraphics.go).
