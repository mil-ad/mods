...
func TestConvoDB(t *testing.T) {
	...
	t.Run("save", func(t *testing.T) {
		...
		require.NoError(t, db.Save(testid, "message 1", "openai", "gpt-4o", 0))
	...
	t.Run("save persists turn count", func(t *testing.T) {
		db := testDB(t)

		require.NoError(t, db.Save(testid, "message 1", "openai", "gpt-4o", 3))
		convo, err := db.Find("df31")
		require.NoError(t, err)
		require.Equal(t, 3, convo.TurnCount)

		require.NoError(t, db.Save(testid, "message 2", "openai", "gpt-4o", 5))
		convo, err = db.Find("df31")
		require.NoError(t, err)
		require.Equal(t, 5, convo.TurnCount)
	})

	t.Run("save no id", func(t *testing.T) {
		db := testDB(t)
		require.Error(t, db.Save("", "message 1", "openai", "gpt-4o", 0))
	})

	t.Run("save no message", func(t *testing.T) {
		db := testDB(t)
		require.Error(t, db.Save(newConversationID(), "", "openai", "gpt-4o", 0))
	})
	...
	t.Run("update", func(t *testing.T) {
		...
		require.NoError(t, db.Save(testid, "message 2", "openai", "gpt-4o", 0))
	...
	t.Run("find head multiple", func(t *testing.T) {
		...
		require.NoError(t, db.Save(nextConvo, "another message", "openai", "gpt-4o", 0))
	...
	t.Run("find by title", func(t *testing.T) {
		...
		require.NoError(t, db.Save(newConversationID(), "message 1", "openai", "gpt-4o", 0))
		require.NoError(t, db.Save(testid, "message 2", "openai", "gpt-4o", 0))
	...
	t.Run("find match many", func(t *testing.T) {
		...
		require.NoError(t, db.Save(testid, "message 1", "openai", "gpt-4o", 0))
		require.NoError(t, db.Save(testid2, "message 2", "openai", "gpt-4o", 0))
	...
	t.Run("completions", func(t *testing.T) {
		...
		require.NoError(t, db.Save(testid1, title1, "openai", "gpt-4o", 0))
		require.NoError(t, db.Save(testid2, title2, "openai", "gpt-4o", 0))
